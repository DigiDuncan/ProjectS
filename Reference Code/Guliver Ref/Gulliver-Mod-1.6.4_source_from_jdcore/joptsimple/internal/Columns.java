...
class Columns
{
  ...
  List<Row> fit(Row row) {
    List<String> options = piecesOf(option, optionWidth);
    List<String> descriptions = piecesOf(description, descriptionWidth);
  ...
  private List<String> piecesOfEmbeddedLine(String line, int width) {
    ...
    for (int end = words.next(); end != -1; end = words.next()) {
      nextPiece = processNextWord(line, nextPiece, start, end, width, pieces);start = end;
    }
